    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden border-2 border-indigo-950 my-6">
        ...
        <div className="p-5 sm:p-6 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          {/* Quick Demo Access by Class */}
          <div className="pt-4 border-t border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase tracking-wider text-slate-700">
                Pilih Akun Demo Siswa per Jenjang:
              </span>
            </div>
            ...
            {/* Quick Demo Student Buttons Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
              {filteredQuickStudents.map((std) => {
                const isGrade9 = std.class_name.toUpperCase().startsWith('IX') || std.class_name.startsWith('9');
                return (
                  <button
                    key={std.id}
                    type="button"
                    onClick={() => handleQuickDemoLogin(std)}
                    className="p-2.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-indigo-50 hover:border-indigo-300 transition-all text-left flex items-center justify-between gap-2 group cursor-pointer"
                  >
                    <div className="flex items-center gap-2 overflow-hidden">
                      ...
                      <div className="overflow-hidden">
                        <span className="block font-bold text-xs text-slate-900 truncate group-hover:text-indigo-900">
                          {std.full_name}
                        </span>
